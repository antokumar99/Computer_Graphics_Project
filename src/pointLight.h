...
public:
    ...
    {
        lightingShader.use();
        int idx = lightNumber - 1;
        std::string base = "pointLights[" + std::to_string(idx) + "].";
        lightingShader.setVec3((base + "position").c_str(),  position);
        lightingShader.setVec3((base + "ambient").c_str(),   ambientOn  * ambient);
        lightingShader.setVec3((base + "diffuse").c_str(),   diffuseOn  * diffuse);
        lightingShader.setVec3((base + "specular").c_str(),  specularOn * specular);
        lightingShader.setFloat((base + "constant").c_str(),  k_c);
        lightingShader.setFloat((base + "linear").c_str(),    k_l);
        lightingShader.setFloat((base + "quadratic").c_str(), k_q);
    }
    void turnOff()
    {
        ambientOn = 0.0;
